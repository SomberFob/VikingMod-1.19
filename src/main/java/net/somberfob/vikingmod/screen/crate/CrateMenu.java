...
public class CrateMenu extends AbstractContainerMenu {
    ...
    public CrateMenu(int id, Inventory inventory, BlockEntity entity) {
        super(ModMenuTypes.CRATE_MENU.get(), id);
        this.blockEntity = entity;
        this.level = inventory.player.level;
        this.container = inventory;

        checkContainerSize(inventory, CrateBlockEntity.CONTAINER_SIZE);
    ...
    private void addSlots(IItemHandler itemHandler) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                this.addSlot(new CrateSlotItemHandler(itemHandler, index, 62 + i * 18, 21 + j * 18));
                index++;
            }
        }
    }
    ...
    private static final int TE_INVENTORY_SLOT_COUNT = CrateBlockEntity.CONTAINER_SIZE;  // must be the number of slots you have!
